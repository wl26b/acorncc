...
| M4 / extra | (this commit) | Lexer: bitwise + compound-assignment + `++` tokens; `OPERATORS` sorted by length at load | Claude | boilerplate — argued the sort over hand-ordering, since the new prefix chains are three deep |
| M4 / extra | (this commit) | Bitwise `& \| ^ << >>`: `BinaryOp` + `TackyBinaryOp` tags, `INFIX_OPS` rows, five codegen templates | **User** | reviewed — user placed all five precedence tiers correctly first time; Claude caught `ShiftRight` emitting `lsr` where C's signed `>>` needs `asr`, and that the obvious probe hid it because the exit status truncates to 8 bits |
| M4 / ch9 | `34f934c` | `resolve.ts`: the duplicate-parameter check | **User** | caught that the first attempt fixed only definitions — `int f(int a, int a);` is a PROTOTYPE and returned early, before the loop; specced the split, since checking applies to every declaration while binding and renaming only apply where there is a body |
| M4 / ch9 | `ad84c9c` | Codegen: **stack arguments** — the outgoing-argument area in `layoutFrame`, args 9+ written at `[sp, #0]` upward, params 9+ read at `[fp, #16]` upward | **User → Claude-reviewed** | supplied the 10-argument `-O0` oracle and walked through why moving `sp` doesn't destroy the caller's bytes; caught `stagingBytes` scaling by 4 on every call rather than once (three ordinary calls gave a 64-byte area), the stack loop iterating `args[0..n-8]` instead of `args[8..n]`, `[fp, #0]` used for the staging area where it would overwrite the saved frame pointer, and the register loop bound changed to `MAX_REG_ARGS` so a two-argument call read past the end |
